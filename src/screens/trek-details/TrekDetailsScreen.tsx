...
import { beginExpedition } from '../../store/slices/expeditionSlice';
import {
  View,
  Text,
  StyleSheet,
...
export const TrekDetailsScreen = ({ navigation, route }: TrekDetailsScreenProps) => {
  ...
  const [isExpeditionModalVisible, setIsExpeditionModalVisible] = useState(false);
  const [downloadProgress, setDownloadProgress] = useState(-1);

  const handleBeginExpedition = (planType: 'official' | 'custom') => {
    ...
    const interval = setInterval(() => {
      setDownloadProgress(prev => {
        if (prev >= 100) {
          ...
          setTimeout(() => {
            setIsExpeditionModalVisible(false);
            setDownloadProgress(-1);
            if (trek) {
              dispatch(beginExpedition({ trek, planType }));
              navigation.navigate('ExpeditionDashboard');
            }
          }, 500);
          return 100;
        }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity
          style={styles.ctaBtn}
          activeOpacity={0.88}
          onPress={() => setIsExpeditionModalVisible(true)}
        >
          <Text style={styles.ctaBtnText}>Begin Expedition</Text>
      ...
      {/* ── Expedition Modal ── */}
      <Modal
        visible={isExpeditionModalVisible}
        transparent
        animationType="fade"
      >
          ...
          <View style={styles.modalContent}>
            {downloadProgress >= 0 ? (
              <View style={styles.downloadingContainer}>
                <Icon name="cloud-download-outline" size={48} color={colors.accent} />
                <Text style={styles.modalTitle}>Downloading Offline Package...</Text>
                <Text style={styles.modalSub}>Maps, routes, and emergency info</Text>
                <View style={styles.modalProgressBarBg}>
                  <View style={[styles.modalProgressBarFill, { width: `${downloadProgress}%` }]} />
                </View>
                <Text style={styles.modalProgressText}>{downloadProgress}% Complete</Text>
              </View>
            ) : (
              <>
                <Text style={styles.modalTitle}>Choose Your Plan</Text>
                <Text style={styles.modalSub}>How would you like to execute this expedition?</Text>
                
                <TouchableOpacity style={styles.planCard} onPress={() => handleBeginExpedition('official')}>
                  <Icon name="shield-check" size={28} color="#A3E635" />
                  <View style={styles.planCardText}>
                    <Text style={styles.planCardTitle}>Official Plan</Text>
                    <Text style={styles.planCardSub}>Follow the standard certified route</Text>
                  </View>
                  <Icon name="chevron-right" size={20} color={colors.muted} />
                </TouchableOpacity>

                <TouchableOpacity style={styles.planCard} onPress={() => handleBeginExpedition('custom')}>
                  <Icon name="pencil-ruler" size={28} color="#38BDF8" />
                  <View style={styles.planCardText}>
                    <Text style={styles.planCardTitle}>My Custom Plan</Text>
                    <Text style={styles.planCardSub}>Use the itinerary generated by AI Planner</Text>
                  </View>
                  <Icon name="chevron-right" size={20} color={colors.muted} />
                </TouchableOpacity>

                <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setIsExpeditionModalVisible(false)}>
